...
func (c *Config) Open(id string, logger log.Logger) (conn connector.Connector, err error) {
	...
	if c.ClientIDFromEnv != "" {
		if c.ClientID != "" {
			return nil, fmt.Errorf("invalid config: clientID or clientIDEnv are exclusive for the OpenShift connector")
		}
		clientID = os.Getenv(c.ClientIDFromEnv)
	}
	...
	if c.ClientSecretFromEnv != "" {
		if c.ClientSecret != "" {
			return nil, fmt.Errorf("invalid config: clientSecret or clientSecretEnv are exclusive for the OpenShift connector")
		}
		clientSecret = os.Getenv(c.ClientSecretFromEnv)
	}
